...
export class PdfHighlighter<T_HT extends IHighlight> extends PureComponent<Props<T_HT>,State<T_HT>> {
            ...
            {(highlightsByPage[String(pageNumber)] || []).map(
              ({ position, id, ...highlight }, index) => {
                ...
                return highlightTransform(
                  ...
                  this.hideTipAndSelection,     // 这个就是 hideTip
  ...
  hideTipAndSelection = () => {   // 这个就是hideTip
    this.setState({
      tipPosition: null,
      tipChildren: null,
    });
  ...
  renderTip = () => {
    ...
    return (
      <TipContainer scrollTop={this.viewer.container.scrollTop} pageBoundingRect={pageBoundingRect}
        style={{
          left:
  ...
  afterSelection = () => {
    ...
    this.setTip(
      ...
      // position: ScaledPosition, content: {text;image;}, hideTipAndSelection: () => void, transformSelection: () => void) => JSX.Element
