            ...
            function Neo4jD3(_selector, _options) {
                ...
                function appendGraph(container) {
                    svg = container.append('svg')
                        ...
                        //    .on("mousewheel.zoom", onWheelZoom)
                        //    .on("DOMMouseScroll.zoom", onWheelZoom)
                ...
                function wheelZoom() {
                    var mouseScale = d3.event.transform.k,
                    ...
                    prevScale = mouseScale;

                    if (!svgScale) {
                        svgScale = 1;
                    }
                    if (status == '放大') {
                        svgScale += options.scaleStep;
                        if (svgScale > options.maxScale) {
                            svgScale = options.maxScale;
                        }
                    }
                    if (status == '缩小') {
                        svgScale -= options.scaleStep;
                        if (svgScale < options.minScale) {
                            svgScale = options.minScale;
                        }
                    }
                    if (status != '平移' && typeof options.onNodeClick === 'function') {
                        options.onWheelZoom(svgScale);
                    }
                    svg.attr('transform', 'translate(' + translate[0] + ', ' + translate[1] + ') scale(' + svgScale + ')');
                }
                ...
                function appendNode() {     //针对节点的相关事件
                    return node.enter()
                        ...
                        .on('contextmenu', function (d) {
                            ...
                            if (typeof options.onNodeRClick === 'function') {
                                options.onNodeRClick(d);
                            }
                        })
                ...
                function appendMultiText(container, posX, width, fontsize, fontfamily) {        //添加节点上的文字
                    ...
                    var mulText = container.append("text")
                        ...
                        .attr("title", function (d) {           //d:["xxxxxxxxxx"]


                            let label = d.labels[0]

                            // if(label.length>4){
                            //    label = label.slice(0,4)+'...'          //只显示前4个字符，后面显示省略号。这就是这里！！！！！！
                            // }
                            // var strs = [label];

                            //根据标签体内 字数的多少来调整font-szie


                            var strs = splitByLine(label, width, fontsize);         //['硬质金', '属压入', '轧件表', '面脱落']

                            /**
                             * 如果圆内文字要全部展示，使用如下代码：
                             * var strs = splitByLine(label,width,fontsize);
                             * 
                             * 如果只展示前几个文字，使用如下代码：
                             * if(label.length>4){
                                    label = label.slice(0,4)+'...'          //只显示前4个字符，后面显示省略号。这就是这里！！！！！！
                            ...
                            return JSON.stringify(strs);        //["硬质金","属压入","轧件表","面脱落"]
                        })

                        .attr("y", function () {
                            var strs = JSON.parse(d3.select(this).attr('title'));
                            var y = (width - strs.length * fontsize) / 2 - 25;
                            return y;
                        })

                        .style("font-size", function (d) {

                            let label = d.labels[0]     //根据字数动态调整font-szie
                            if (label.length >= 9) {
                                fontsize = 10
                    ...
                    mulText.selectAll("tspan")
                        .data(function () {
                            let res = JSON.parse(d3.select(this).attr('title'))
                            return res;
                            // let res = d3.select(this).attr("title")
                            // console.log("res---", res)
                            // return res
                        })
                        .enter()
                        ...
                        .attr("x", function (d, i, arr) {
                            ...
                            return parseInt(mulText.attr("x")) + (width - curLen) / 2
                        })
                        .attr("dy", "1em")
                        .text(function (d, idx, data) {
                            // console.log("mulText", d)
                            return d;
                        });
                ...
                function appendOutlineToNode(node) {
                    return node.append('circle')
                        ...
                        .style('fill', function (d) {
                            return options.nodeOutlineFillColor ? options.nodeOutlineFillColor : class2color(d.labels[0], d);
                        })
                ...
                function appendRingToNode(node) {       //这是node外侧的圆环
                    return node.append('circle')
                        .attr('class', 'ring')
                ...
                function appendRelationship() {     //这是节点之间连接的 事件处理
                    return relationship.enter()
                        ...
                        .on("contextmenu", function (d, i) {
                            d3.event.preventDefault();
                            if (typeof options.onRelationshipRClick === 'function') {
                                options.onRelationshipRClick(d);
                            }
                        })
                        .on('dblclick', function (d) {      //处理右键点击函数
                            console.log("产生原因 被双击")
                            if (typeof options.onRelationshipDoubleClick === 'function') {
                                options.onRelationshipDoubleClick(d);
                            }
                        })
                        .on('mouseenter', function (d) {        //当鼠标进入 "产生原因" 字段的时候触发
                            //    if (info) {
                            //        updateInfo(d);
                            //    }
                ...
                function appendOverlayToRelationship(r) {           //overlay 覆盖
                ...
                function appendTextToRelationship(r) {          //控制 “产生原因” 字体的样式
                ...
                function class2color(cls, data) {
                    ...
                    if (!color) {
                        ...
                        if (data && data.nodeType) {
                            ...
                            if (clrMap[data.nodeType] === undefined) {
                                color = options.colors[numClasses % options.colors.length];
                            } else {
                                color = options.colors[clrMap[data.nodeType]]
                            }
                        } else {
                            color = options.colors[numClasses % options.colors.length];
                        }
                ...
                function colors() {
                    return [
                        // 缺陷:fff7ef--fbb07d  F48617
                        // 报警:fbe6e6--fd8686  FF3333
                        // 轧制阶段原因:e4e4fd--6585f7  5075F7
                        // 物料原因:e9f7ef--57c186  46C881
                        // 连铸阶段原因:e4effd--2b86f9  007CEA
                        '#F48617',
                        '#FF3333',
                        '#5075F7',
                        '#46C881',
                ...
                function modDragged(d) {
                    d.fx = d.x + 5;
                    d.fy = d.x + 5;
                }

                function modeDragStarted(d) {
                    simulation.alphaTarget(0.3).restart();
                    d.fx = d.x;
                    d.fy = d.y;
                }
                ...
                var tickEndTimer = null;
                var isTicking = false;
                var tickDoOnce = false;
                ...
                function initSimulation() {
                    var simulation = d3.forceSimulation()
                        ...
                        .on('tick', function () {
                            ...
                            delayEndTick(options.maxAnimateTime);
                    ...
                    function endTick() {
                        clearTimeout(tickEndTimer);
                        tickEndTimer = setTimeout(function () {
                            if (isTicking) {
                ...
                function rotate(cx, cy, x, y, angle) {
                    ...
                    // console.log("cx, cy, x, y, angle", cx, cy, x, y, angle)
                    ...
                    // console.log("{ x: nx, y: ny }", { x: nx, y: ny })
                ...
                */
                ...
                function selectNode(id) {
                    svgNodes.selectAll('.node').each(function (d) {
                        var rel = d3.select(this);
                        var oldClass = rel.attr('class');
                        if (d.id == id) {
                            if (oldClass.indexOf('sel') == -1) {
                                rel.attr('class', oldClass + ' sel');
                            }
                        } else if (oldClass.indexOf('sel') != -1) {
                            rel.attr('class', oldClass.replace('sel', ''));
                        }
                    })
                }
                ...
                function zoomFit(scale) {
                    ...
                    // scale = 0.85 / Math.max(width / fullWidth, height / fullHeight);
                    svgScale = scale;
                    svgTranslate = [fullWidth / 2 - scale * midX, fullHeight / 2 - scale * midY];

                    svg.attr('transform', 'translate(' + svgTranslate[0] + ', ' + svgTranslate[1] + ') scale(' + scale + ')');
                ...
                return {
                    ...
                    modeDragStarted: modeDragStarted,
                    modDragged: modDragged,
                    modDragEnded: modDragEnded,
                    version: version,
